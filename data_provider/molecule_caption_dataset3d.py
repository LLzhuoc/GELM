...
class MoleculeCaption_double(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ####信息熵
        target_name_list = os.listdir(self.root+'target/'+str(index)+'/')
        target = os.path.join(self.root, 'target/'+str(index)+'/',target_name_list[0])
        y = torch.load(target)
        ####信息熵
        ...
        return data_graph1,data_graph2, text ,smiles_prompt,y
...
class MoleculeCaption_double_consel(Dataset):
    def __init__(self, root, text_max_len, dictionary=None,prompt=None,zijiegou=False,autozijiegou=False,solve=False):
        super(MoleculeCaption_double_consel, self).__init__(root)
        self.root = root
        self.text_max_len = text_max_len
        self.tokenizer = None
