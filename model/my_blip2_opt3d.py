...
class SubgraphSelector(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_nodes_to_select=10, 
                 lambda_mi=1.0, lambda_se=0.5):
        ...
        self.lambda_se = lambda_se  # 结构熵损失权重
    ...
    def build_graph(self, features, similarity_threshold=0.5):
        """构建基于特征相似度的邻接矩阵"""
        ...
        # 应用阈值
        adj_matrices = (similarity > similarity_threshold).float()
        
        # 移除自环（对角线）
        eye = torch.eye(num_nodes, device=features.device).unsqueeze(0).expand(batch_size, -1, -1)
        adj_matrices = adj_matrices * (1 - eye)
        
        return adj_matrices
    
    def select_exact_k_nodes(self, scores, k):
        """选择恰好k个评分最高的节点并返回索引和掩码"""
        ...
        for b in range(batch_size):
            # 获取前k个最高评分的索引
            if k > num_nodes:
                # 如果k大于num_nodes，则重复选择一些节点以达到k个
                indices = torch.argsort(scores[b], descending=True)
                repeated_indices = torch.cat([indices] * (k // num_nodes + 1))
                top_indices = repeated_indices[:k]
            else:
                # 正常情况：选择前k个
                _, top_indices = torch.topk(scores[b], k)
            
            # 将这些索引设为1
            mask[b, top_indices] = 1.0
            all_indices.append(top_indices)
            
        return mask, all_indices
    
    def extract_subgraph_adj(self, full_adj, node_indices):
        """从完整邻接矩阵中提取子图邻接矩阵"""
        batch_size = len(node_indices)
        k = self.num_nodes_to_select
        
        # 创建子图邻接矩阵
        subgraph_adj = torch.zeros(batch_size, k, k, device=full_adj.device)
        
        for b in range(batch_size):
            indices = node_indices[b]
            # 从完整邻接矩阵中提取选定节点之间的连接
            subgraph_adj[b] = full_adj[b][indices, :][:, indices]
        
        return subgraph_adj
    
    def compute_structural_entropy(self, adj_matrices):
        """计算结构熵
        
        结构熵是基于节点的度分布和连接模式来衡量图的复杂性的。
        这里我们使用一阶结构熵，它基于节点度的分布。
        
        参数:
            adj_matrices: 形状为 [batch_size, num_nodes, num_nodes] 的邻接矩阵
        ...
        for b in range(batch_size):
            ...
            total_edges = degrees.sum() / 2  # 无向图中边的总数
            
            if total_edges == 0:
                # 如果没有边，结构熵为0
                entropies.append(torch.tensor(0.0, device=adj.device))
                continue
                
            # 计算每个节点的归一化度 (作为概率)
            probs = degrees / (2 * total_edges)
            
            # 移除度为0的节点（log(0)是未定义的）
            probs = probs[probs > 0]
    ...
    def compute_graph_diversity(self, subgraph_adj):
        """计算子图的多样性
        
        子图多样性基于连接模式的均匀性。我们希望选择的子图在结构上是多样的，
        而不是所有节点都在相同的密集连接社区中。
        
        参数:
            subgraph_adj: 形状为 [batch_size, k, k] 的子图邻接矩阵
            
        返回:
            多样性得分 (越高表示多样性越好)
        """
        batch_size, k, _ = subgraph_adj.shape
        diversity_scores = []
        ...
        for b in range(batch_size):
            ...
            # 计算子图的聚类系数
            clustering = torch.zeros(k, device=adj.device)
            for i in range(k):
                ...
                # 计算邻居之间的连接数
                neighbor_connections = adj[neighbors, :][:, neighbors].sum() / 2
                possible_connections = len(neighbors) * (len(neighbors) - 1) / 2
                
                if possible_connections > 0:
                    clustering[i] = neighbor_connections / possible_connections
            
            # 我们希望聚类系数的方差较大，表示节点来自不同的社区
            diversity = clustering.var()
            diversity_scores.append(diversity)
        
        # 返回批次的平均多样性
        return torch.stack(diversity_scores).mean()
    
    def structural_entropy_loss(self, full_adj, node_indices):
        """计算结构熵损失
        
        结合了结构熵和多样性来评估子图的质量。
        我们希望子图既要有信息量，同时又要捕捉图的多样性。
        
        参数:
            full_adj: 完整邻接矩阵
            node_indices: 选择的节点索引
            
        返回:
            结构熵损失
        """
        # 提取子图邻接矩阵
        subgraph_adj = self.extract_subgraph_adj(full_adj, node_indices)
        
        # 计算结构熵 (我们希望它较低，表示有规律性)
        entropy = self.compute_structural_entropy(subgraph_adj)
        
        # 计算多样性 (我们希望它较高，表示多样性)
        diversity = self.compute_graph_diversity(subgraph_adj)
    ...
    def forward(self, features, similarity_threshold=0.5):
        """
        输入:
            features: [batch_size, num_nodes, feature_dim]
        输出:
            total_loss: 总损失（互信息损失 + 结构熵损失）
            selected_features: 被选中的子图节点特征 [batch_size, num_nodes_to_select, hidden_dim]
            selection_mask: 节点选择掩码
            losses_dict: 各个损失项的字典
        """
        batch_size, num_nodes, feature_dim = features.shape
        
        # 验证我们是否有足够的节点
        if num_nodes < self.num_nodes_to_select:
            print(f"警告：输入节点数 ({num_nodes}) 小于要选择的节点数 ({self.num_nodes_to_select})。将选择所有节点并重复一些节点。")
        
        # 构建邻接矩阵
        adj_matrices = self.build_graph(features, similarity_threshold)
        
        # 为了方便GIN处理，需要将每个batch分开处理
        ...
        for b in range(batch_size):
            ...
            if edge_index[0].size(0) == 0:
                # print(f"警告: 批次 {b} 没有边，添加自环作为替代")
                edge_index = torch.stack([
                    torch.arange(num_nodes, device=features.device),
                    torch.arange(num_nodes, device=features.device)
                ])
            else:
            ...
            # 应用GIN进行特征聚合
            aggregated_features = self.gin_conv(node_features, edge_index)
            all_aggregated_features.append(aggregated_features)
        
        # 将所有batch的结果堆叠起来
        aggregated_features = torch.stack(all_aggregated_features)
        
        # 应用节点选择器得到评分
        selection_scores = self.node_selector(aggregated_features).squeeze(-1)
        
        if self.training:
            # 训练时使用独热编码和Gumbel技巧
            # 计算Top-K的掩码和索引
            hard_mask, selected_indices = self.select_exact_k_nodes(selection_scores, self.num_nodes_to_select)
            
            # 计算软掩码（用于梯度传播）
            selection_probs = F.softmax(selection_scores, dim=1)
            
            # 创建混合掩码：前向传播时用硬掩码，反向传播时用软掩码
            selection_mask = hard_mask.detach() + selection_probs - selection_probs.detach()
            
            # 为了MI计算准备完整形状的特征（所有节点，未选中的为0）
            full_selected_features = aggregated_features * selection_mask.unsqueeze(-1)
            
            # 计算互信息损失
            mi_loss = self.donsker_varadhan_mi_loss(aggregated_features, full_selected_features)
            
            # 计算结构熵损失
            se_loss = self.structural_entropy_loss(adj_matrices, selected_indices)
            
            # 提取选定节点的特征，形状为 [batch_size, num_nodes_to_select, hidden_dim]
            compact_selected_features = torch.zeros(
                batch_size, self.num_nodes_to_select, self.hidden_dim, 
                device=aggregated_features.device
            )
            
            for b in range(batch_size):
                # 由于在训练时我们使用了混合掩码，这里仍然使用hard_mask的索引
                indices = selected_indices[b]
                compact_selected_features[b] = aggregated_features[b, indices]
        
        else:
            # 测试时直接选择恰好k个最高评分的节点
            selection_mask, selected_indices = self.select_exact_k_nodes(selection_scores, self.num_nodes_to_select)
        ...
        # 计算总损失
        total_loss = self.lambda_mi * mi_loss + self.lambda_se * se_loss
        ...
        return total_loss, compact_selected_features, selection_mask, losses_dict
    
    def donsker_varadhan_mi_loss(self, x, y):
        """使用Donsker-Varadhan方法近似计算互信息"""
        batch_size, num_nodes, feature_dim = x.shape
        
        # 将特征展平为[batch_size * num_nodes, feature_dim]
        x_flat = x.reshape(batch_size * num_nodes, -1)
        y_flat = y.reshape(batch_size * num_nodes, -1)
        
        # 计算正样本得分
        pos_scores = self.mi_estimator(torch.cat([x_flat, y_flat], dim=1))
        
        # 打乱第二个特征以获取负样本
        idx_perm = torch.randperm(batch_size * num_nodes)
        y_perm = y_flat[idx_perm]
        
        # 计算负样本得分
        neg_scores = self.mi_estimator(torch.cat([x_flat, y_perm], dim=1))
        
        # Donsker-Varadhan表示形式
        mi_loss = -(torch.mean(pos_scores) - torch.log(torch.mean(torch.exp(neg_scores))))
        
        return torch.clamp(mi_loss,0)
...
class Blip2OPT(Blip2Base):
    ...
    'additional_special_tokens': ['[START_I_SMILES]', '[END_I_SMILES]','</s>']
        ...
        self.opt_tokenizer.add_special_tokens({
    'additional_special_tokens': ['<start_property>', '<end_property>']
        })
        ...
        self.subgraph = SubgraphSelector(input_dim=512, 
        hidden_dim=512, 
        num_nodes_to_select=self.args.num_query_token,
        lambda_mi=0,
        lambda_se=0.1)

        self.fenlei = nn.Linear(512,1)

    def forward(self, batch):
        
        graph1, graph2,prompt_tokens, text_tokens,y= batch
        y = torch.stack(y)
        y=y.unsqueeze(1)
        # print('&'*50,y)
        # print('&'*50,y.shape)
        ...
        for i in range(len(graph1)):
            ...
            graph1_embeds = torch.mean(graph1_embeds[:20],dim=1)
            embeds_list.append(graph1_embeds)
        #batchsize*numquery*512
        graphconformer1 = torch.stack(embeds_list,dim=0)
        graphconformer1 = graphconformer1.to(torch.bfloat16)
        # graphconformer1 = self.graemb1m(graphconformer1)
        total_loss1, graphconformer1, _, _ = self.subgraph(graphconformer1)
        sc1 = graphconformer1.mean(dim=1)
        # sc1 = sc1.to(torch.bfloat16) 
        sc1 = self.fenlei(sc1)
        ...
        for i in range(len(graph2)):
            ...
            graph2_embeds = torch.mean(graph2_embeds[:20],dim=1)
            embeds_list.append(graph2_embeds)

        graphconformer2 = torch.stack(embeds_list,dim=0)
        graphconformer2 = graphconformer2.to(torch.bfloat16)
        # graphconformer2 = self.graemb2m(graphconformer2)
        total_loss2, graphconformer2, _, _ = self.subgraph(graphconformer2)
        sc2 = graphconformer2.mean(dim=1)
        # sc2 = sc2.to(torch.bfloat16) 
        sc2 = self.fenlei(sc2)
        ...
        #Prompt | text
        #smilesA molA smilesB molB | DrugsA and ...
        ...
        lossfenlei = criterion(sc1, y)+criterion(sc2,y)

        # print(total_loss,lossfenlei,outputs.loss)
        loss = outputs.loss+0.0001*(lossfenlei+total_loss)
    ...
    def generate(
        ...
        # graphconformer1 = graphconformer1.to(torch.bfloat16)
        # graphconformer1 = self.graemb1m(graphconformer1)
        _, graphconformer1, _, _ = self.subgraph(graphconformer1)
        ...
        # graphconformer2 = graphconformer2.to(torch.bfloat16)
        # graphconformer2 = self.graemb2m(graphconformer2)
        _, graphconformer2, _, _ = self.subgraph(graphconformer2)
